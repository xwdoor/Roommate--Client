package net.xwdoor.roommate.adapter.superAdapter;
...
/**
 * Created by XWdoor on 2016/4/15 015 9:49.
 * 博客：http://blog.csdn.net/xwdoor
 */
public class SuperViewHolder implements IChainSetter<SuperViewHolder> {

    public final View mItemView;
    private SparseArray<View> mChildViews;

    private SuperViewHolder(View view) {
        mItemView = view;
        mChildViews = new SparseArray<>();
    }

    public static SuperViewHolder getHolder(View convertView, LayoutInflater inflater, int layoutResId, ViewGroup parent) {
        SuperViewHolder holder;
        if (convertView == null) {
            //这里必须添加第三个参数，不然报错
            //java.lang.UnsupportedOperationException: addView(View, LayoutParams) is not supported in AdapterView
            View view = inflater.inflate(layoutResId, parent, false);
            holder = new SuperViewHolder(view);
            view.setTag(holder);
        } else {
